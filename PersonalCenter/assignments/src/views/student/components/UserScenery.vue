...
const fetchFoods = async () => {
    try {
        ...
        const response = await axiosInstance.get('/api/scenery', {
            data: formData,
            headers: {
...
const updateFood = async () => {
    try {
        ...
        const response = await axiosInstance.put('/api/scenery',formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
...
const addFood = async () => {
    try {
        ...
        const response = await axiosInstance.post('/api/scenery', formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
...
const deleteFood = async () => {
    try {
        ...
        formData.append('email', food.value.email);
        formData.append('name', food.value.name);
        const response = await axiosInstance.delete('/api/scenery', {
            data: formData,
            headers: {
                'Content-Type': 'multipart/form-data',
            },
...
const searchFood = async () => {
    try {
        ...
        console.error('无法查找scenery:', error);
    }
}
const routeToCard = (item: Food) => {
    ...
    if (selectedFood) {
        ...
        console.log('选中的景点:', selectedFood);
    } else {
        console.error('未找到对应的景点');
    }
};
const switchFood = () => {
    if (isShow) {
