...
const updateFood = async () => {
    try {
        ...
        formData.append('material', food.value.material);
        ...
        const response = await axiosInstance.put('/api/foods',
            formData,{
            headers: {
                'Content-Type': 'multipart/form-data',
            },
...
const deleteFood = async () => {
    try {
        ...
        formData.append('email', food.value.email);
        formData.append('name', food.value.name);
        const response = await axiosInstance.delete('/api/foods', {
            data: formData,
            headers: {
                'Content-Type': 'multipart/form-data',
            },
...
const routeToCard = (item: Food) => {
    ...
    if (selectedFood) {
        // 这里可以执行任何关于找到的selectedFood的操作
        food.value = selectedFood;  // 比如将当前food对象更新为选中的food
        console.log('选中的美食:', selectedFood);
    } else {
        console.error('未找到对应的美食');
    }
};
const switchFood = () => {
    if (isShow) {
        ...
        food.value = {
            ...
            description: '',
            img: '',
            email: user.value.email, // 使用当前用户的email
        };
    } else {
        isShow = !isShow;
        food.value = foods[0]
