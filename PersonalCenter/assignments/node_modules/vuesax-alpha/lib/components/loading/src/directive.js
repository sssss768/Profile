...
const createInstance = (el, binding) => {
  ...
  const getProp = (name, def) => {
    ...
    return resolveExpression(
      (_b = (_a2 = getBindingProp(name)) != null ? _a2 : el.getAttribute(`element-loading-${shared.hyphenate(name)}`)) != null ? _b : def
    );
  };
  const options = {
    text: getProp("text"),
  ...
  el[INSTANCE_KEY] = {
    options,
    instance: method.loading(options)
  };
};
...
const vLoading = {
  mounted(el, binding) {
    if (binding.value == true || shared.isObject(binding.value) && binding.value.visible) {
  ...
  updated(el, binding) {
    ...
    if (!lodashUnified.isEqual(binding.value, binding.oldValue)) {
